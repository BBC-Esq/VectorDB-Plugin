...
### What does the Show Thinking checkbox do when querying?
Some chat models are 'reasoning' or 'thinking' models that work through a problem internally before giving their final answer,
wrapping that internal reasoning in special tags. The 'Show Thinking' checkbox in the Query Database Tab controls whether you see
that reasoning. When it is unchecked (the default), the internal thinking is hidden and you see only the final answer, though a
small animated 'Thinking...' indicator appears while the model is reasoning so you know it is working. When checked, the full
thought process is displayed along with the answer. This setting only affects reasoning models; for ordinary models that do not
produce a separate thinking step, it is simply ignored.

### What do the Copy Response and Eject buttons do in the Query tab?
The Query Database Tab includes convenience controls beyond the question box. The 'Copy Response' button copies the current answer
(or the retrieved chunks, if 'Chunks Only' is checked) to your clipboard as plain text so you can paste it elsewhere. When you are
using the Local Models backend, an 'Eject' button appears that unloads the currently loaded chat model from memory; this frees up
VRAM without closing the program, which is handy when you want to switch to creating a database or simply reclaim memory. The
model is loaded again automatically the next time you submit a question with a local model selected.
...
API Key.' Then, in the Query Database Tab, choose one of the MiniMax options from the backend dropdown and submit your question as
usual. MiniMax offers several model variants (including a high-speed option) that you select directly from that dropdown. It is a
good choice if you want access to a powerful hosted model without running anything locally.
...
MiniMax API key is entered from the File menu). Click OK to save.
